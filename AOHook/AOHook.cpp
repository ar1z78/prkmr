 ...
 * MishRoller into AO process.
 *
 * Note that you have to compile this project separately.
 * If you set it as dependance of MishRoller, it will
 * tell the linker to link MishRoller with AOHook.lib,
 * which doesn't exist.
 *
 * The output path for the dll is the MishRoller sources
...
Message_t* DataBlockToMessageHook( int _Size, void* _pDataBlock )
{
    ...
    if( _Size > 0x40 )
    {
        ...
        */
        if( Temp == 0xc3da0000 )
        {
            // Find MishRoller's hook thread window and send the datas
            // using WM_COPYDATA
            if( hWnd = FindWindow( "Ar1zMishRollerHookWindowClass", "Ar1zMishRollerHookWindow" ) )
            {
                COPYDATASTRUCT Data;
                Data.cbData = _Size;
